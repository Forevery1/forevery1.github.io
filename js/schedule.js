;(function (undefined) {
    ...
    function Schedule(opt) {
        var def = {}, opt = extend(def, opt, true), curDate = opt.date ? new Date(opt.date) : new Date(),
            ...
            nDate = opt.nDate || "", el = document.querySelector(opt.el) || document.querySelector('body'), _this = this;
        ...
        var render = function () {
            ...
            for (var i = 0; i < total; i++) {
                if (i < startWeek) {
                    ...
                    for (var k = 0; k < nDate.length; k++) {
                        nDate[k].time == nowDate && (addClass = 'ls_day', Morning = nDate[k].Morning, Afternoon = nDate[k].Afternoon);
                    }
                    eleTemp.push('<li class="current-month" ><span  class="currentDate dayStyle ' + addClass + '">' + (i + 1 - startWeek) + '</span><div class="day_time" style="display: none"><div>上班：' + Morning + '</div><div>下班：' + Afternoon + '</div></li>')
                } else {
